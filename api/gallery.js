module.exports = async function handler(req, res) {
  ...
  // Search in previews or edited subfolder
  const subfolder = type === 'edited' ? 'edited' : 'previews';
  const searchPrefix = `clients/${folder}/${subfolder}`;

  try {
    const response = await fetch(
      `https://api.cloudinary.com/v1_1/${cloudName}/resources/image?prefix=${encodeURIComponent(searchPrefix)}&max_results=50&type=upload`,
      {
